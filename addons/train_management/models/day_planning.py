...
class DayPlanning(models.Model):
    ...
    day_planning_shifts_allocated_count = fields.Integer(compute="_compute_sum_of_shifts_allocated")
    ...
    @api.depends("day_planning_shift_ids")
    def _compute_sum_of_shifts_allocated(self):
        for record in self:
            record.day_planning_shifts_allocated_count = len(
                [shift for shift in record.day_planning_shift_ids if shift.person])
